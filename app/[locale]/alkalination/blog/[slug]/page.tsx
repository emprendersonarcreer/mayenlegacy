...
export default async function AlkalinationBlogPost({
  params,
}: {
  params: Promise<{ locale: string; slug: string }>;
}) {
    ...
    <main className="min-h-screen bg-[#FAF7F2]">
      ...
      <article className="pt-36 pb-20 px-6 max-w-2xl mx-auto">
        ...
        <div className="flex items-center gap-2 text-xs text-gray-400 tracking-widest mb-10">
          <Link
            href="/alkalination/blog"
            className="hover:text-[#4A90E2] transition-colors"
          >
            BLOG
          </Link>
          <span>›</span>
          <span className="text-[#4A90E2] truncate">{post.title.toUpperCase()}</span>
        </div>
        ...
        {/* Divider */}
        <div className="w-12 h-[1px] bg-[#4A90E2] mb-10" />

        {/* Body */}
        {post.body && (
          <div className="prose prose-lg max-w-none text-gray-700 leading-relaxed prose-headings:font-serif prose-headings:text-[#2C4A7C] prose-a:text-[#4A90E2] prose-blockquote:border-l-[#4A90E2] prose-blockquote:text-[#2C4A7C] prose-blockquote:italic">
