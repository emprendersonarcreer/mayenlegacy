...
export default async function EssentiaBlogPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  return (
    <main className="min-h-screen bg-[#FAF7F2]">
      ...
      <section className="pt-40 pb-16 px-6 text-center">
        <p className="text-[#9A7B0A] tracking-[0.4em] text-xs mb-4">
          {t("hero.tagline")}
        </p>
        <div className="w-12 h-[1px] bg-[#9A7B0A] mx-auto mb-6" />
        <h1 className="font-serif text-4xl sm:text-6xl text-[#2C4A7C] tracking-widest mb-4">
          {t("hero.title")}
        </h1>
        <div className="w-12 h-[1px] bg-[#9A7B0A] mx-auto mt-6" />
      ...
      <section className="py-12 px-6 max-w-5xl mx-auto">
        {posts.length === 0 ? (
          <div className="text-center py-24">
            <p className="text-[#9A7B0A] tracking-[0.3em] text-sm mb-3">
              {t("empty.label")}
            </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {posts.map((post) => (
              <Link
                key={post._id}
                href={`/essentia/blog/${post.slug.current}`}
                className="group bg-white border border-[#EADCC6] hover:border-[#9A7B0A] transition-colors"
              >
                ...
                <div className="p-6">
                  ...
                  <h2 className="font-serif text-xl text-[#2C4A7C] mb-2 group-hover:text-[#9A7B0A] transition-colors">
                    {post.title}
                  </h2>
                  {post.excerpt && (
                  ...
                  <p className="text-[#9A7B0A] text-xs tracking-widest mt-4">
                    {t("readMore")} →
                  </p>
                </div>
              </Link>
